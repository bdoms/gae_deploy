GAE Deploy is a project to minify CSS and JavaScript before deploying, and handle static file cache-busting in production.
...
The command line arguments are just a list of directories (relative or absolute) which contain CSS and JS files.
If you have a single directory with all your static or public assets, this becomes even simpler:

python gae_deploy public

There is a single command line option, which is to specify a folder that the built URLs should be relative to.
